...
struct ErrorHandler {
    ...
    static func checkAuthErrorSSO(_ response: Response) -> AuthErrorType? {
        ...
        if response.statusCode == 401 {
            if let result = try? JSONDecoder().decode(ErrorModel.self, from: response.data) {
                if result.needReLogin {
                    StatusManager.shared.stopAllLoading()
                    AlertManager().showAlertAuthError()
                    return .ReLogin
                } else {
                    fLog("토큰 다시 받음: \(result.accessToken)")
                    UserManager.shared.setLoginDataSSO(accessToken: result.accessToken)
                    return .Refreshed
                }
            }
            return .ReLogin

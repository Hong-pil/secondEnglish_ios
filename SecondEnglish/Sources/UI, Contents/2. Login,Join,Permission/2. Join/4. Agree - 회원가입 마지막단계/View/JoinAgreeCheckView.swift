    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(spacing: 0) {
                Button {
                    ...
                    HStack(alignment: .center, spacing: 0) {
                        Image(self.checkAll ? "Checkbox_login_checked" : "delegate_unchecked")
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                //이벤트 정보 수신 동의 (선택)
                JoinAgreeCheckSubView(isCheck: $checkAgreeAd, title: "a_agree_recieve_event_optional".localized, isOptional: true, isMore: false) { type in
                    if type == .Check {
                        checkAgreeAd.toggle()
                        checkAgree()
                    }
